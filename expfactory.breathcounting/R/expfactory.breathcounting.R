...
#' Group Breath Counting Accuracy
#'
#' Process ePrime breath counting data for n participants
#' @param df Data frame. Breath counting accuracy for n participants
#' @param c String. Column to group by
#' @importFrom dplyr group_by_at ungroup group_modify
#' @export
#'
n_breath_counting_accuracy <- function(df, c='subject') {
  # group_modify() functions require >= 2 args
  # this shim avoids changing the breath_counting_accuracy() API
  shim <- function(.x, .y) {
    breath_counting_accuracy(.x)
  }
  accuracy <- df %>%
    group_by_at(c) %>%
    group_modify(shim) %>%
...
#' Convert ePrime or Experiment Factory breath counting data to CSV
#'
#' Convert ePrime or Experiment Factory breath counting data to CSV
...
breath_counting_to_csv <- function(data_path, type='expfactory', format='json',
                                   participants, t, exclude) {
  if (type == 'eprime') {
    df <- eprime_breath_counting_to_csv(data_path)
  } else if (type == 'expfactory') {
    if (format == 'csv') {
      df <- expfactory_breath_counting_to_csv(data_path, t, exclude)
    } else { # expfactory, format == json
      df <- expand.grid(token = participants$token) %>%
        mutate(path=paste0(data_path, '/', token, '_finished/breath-counting-task-results.json')) %>%
        group_by(path) %>%
        do(., process_breath_counting(.$path, .$token)) %>%
        rename(token = p) %>%
        ungroup()
      df <- left_join(participants, df, by = 'token') %>%
        select(p, Sample, response, rt)
      write.csv(df, file=paste0(data_path,'/bc.csv'), row.names = FALSE)
    }
  }
  return(df)
}
