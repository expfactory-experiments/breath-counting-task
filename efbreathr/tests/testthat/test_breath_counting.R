...
test_that("process_breath_counting() can process a JSON file", {
  expect_silent(efbreathr::process_breath_counting(path, p=1))
})
bc <- efbreathr::process_breath_counting(path, p=1)
...
test_that("breath_counting_accuracy() returns a data frame", {
  expect_is(efbreathr::breath_counting_accuracy(df), "data.frame")
})
foo <- efbreathr::breath_counting_accuracy(df)
